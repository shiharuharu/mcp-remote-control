"""CLI isomorphic tool subcommands (T05/T06)."""
...
from mcp_remote_control.core import console_ops, endpoint_ops
...
def test_screen_list_and_ps_local_unsupported(
    ...
    # T09: screen list is implemented (empty registry → ok n=0).
    assert main(["screen", "list"]) == EXIT_OK
    out = capsys.readouterr().out
    assert out.startswith("@screen ok")
    assert "n=0" in out

    # T14: ps on local → UNSUPPORTED (winrm-only).
...
# ---------------------------------------------------------------------------
# O10: console + config CLI surface (param pass-through, hyphenated help,
...
def test_console_open_happy_mocked_serial(
    ...
    out = capsys.readouterr().out
    assert out.startswith("@console ok")
    # Core reliably renders ``path=COM9``; the weak ``or "COM9"`` fallback
    # masked a regression where path= stopped being reflected.
...
def test_config_put_secret_content_file_missing(
    ...
    Pins MED 1's fix: before, ``_resolve_secret_content`` raised a raw
    ``FileNotFoundError`` that escaped ``cli.main`` as an unhandled traceback
    (exit 1). After the fix, the CLI surfaces a clean ``@config error
    code=INVALID_ARG`` OpResult and returns ``EXIT_VALIDATION`` (3) — matching
    how ``--winrm-json`` bad-JSON is handled.
    """
    home = tmp_path / "mrc"
    monkeypatch.setenv("MRC_HOME", str(home))
    reset_registry()
    ...
    # No unhandled exception leaked as a Python traceback before the fix
    # (the exc type name in the clean msg is fine; the traceback is the pin).
...
# ---------------------------------------------------------------------------
# O12: EXIT_TRANSPORT=4 mapping for transport-code OpResults (cli_cmds/tools._exit_for)
# ---------------------------------------------------------------------------
